...
export const addStudent = async (req, res) => {
  ...
  try {
    // 1. Create the student (wrapped in array for session execution)
    const [student] = await Student.create([{ ...req.body }], { session });

    // 2. Safely push the student ID to the Batch
    if (student.batch) {
    ...
    if (error.code === 11000) {
      return res.status(400).json({ message: `Duplicate key error: Student ID or Registration exists` });
    }
...
export const updateStudent = async (req, res) => {
  ...
  try {
    ...
    // Handle Batch Transfer Logic safely
...
// ... keep your getAllStudents and search functionality as is ...
