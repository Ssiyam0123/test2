...
export const deleteUser = async (req, res) => {
  try {
    ...
    // DANGER: We do not do await user.deleteOne() anymore!
    // Instead, we scrub their access and mark them as inactive (Soft Delete).
    // This preserves their name on all past financial and inventory logs.
    
    user.status = "Resigned";
    user.is_active = false; 
    
    // Scramble their password so they can't log in, and remove their image to save space
    ...
    res.status(200).json({ message: "User access revoked and account archived." });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};
