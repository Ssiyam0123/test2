...
import { getStudentFormSchema } from "../../validators/zodSchemas.js"; // 🚀 Use dynamic schema
...
const AddStudentForm = ({ mode = "add", data = null }) => {
  ...
  const { data: coursesData, isFetching: coursesFetching, error: coursesError } = useActiveCourses(queryFilter);
  const { data: batchesData, isFetching: batchesFetching, error: batchesError } = useBatches(queryFilter);
  ...
  const branchOptions = useMemo(() => {
    ...
    return availableBranches.map(b => ({ value: b._id, label: `${b.branch_name} (${b.branch_code})` }));
  ...
  const batchOptions = useMemo(() => {
    ...
    return allBatches.map((b) => ({ value: b._id, label: `${b.batch_name} (${b.course?.course_name || 'N/A'})` }));
  }, [batchesData]);
  ...
  const initialData = useMemo(() => {
    ...
    return {
      ...
      gender: "male", course: "", batch: "",
      issue_date: today, contact_number: "", email: "", address: "",
      branch: branchId || ""
    };
  ...
  const studentConfig = [
    ...
    { name: "course", label: "Course", type: "select", defaultOption: coursesFetching ? "Loading courses..." : "Select a course...", options: courseOptions, required: true },
    { name: "batch", label: "Batch", type: "select", defaultOption: batchesFetching ? "Loading batches..." : "Select a batch...", options: batchOptions, required: true },
  ...
  const handleSubmit = (formData, jsonPayload) => {
    ...
    // 🚀 Fix form-data parsing issue for checkboxes when adding
    if (mode === "add") {
