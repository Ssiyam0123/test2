...
export default function CurriculumBuilderModal({ batch, onClose }) {
  ...
  const handleImport = () => {
    ...
    const importedRows = masterData.map((item, idx) => ({
      topic: item.topic,
      // 🚀 Map any inconsistent master data to strict backend enums
  ...
  const handleSubmit = (e) => {
    ...
    const finalData = rows
      ...
      .map(r => ({
        ...
        class_number: Number(r.order_index), // Both included for safety
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 shrink-0">
          ...
          <button onClick={onClose} className="p-2 hover:bg-rose-50 hover:text-rose-500 rounded-full transition-colors"><X size={24} /></button>
        </div>

        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-slate-50/30">
          {view === "selection" ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-10 max-w-3xl mx-auto">
              <button onClick={handleImport} className="flex flex-col items-center text-center p-10 bg-indigo-50 border-2 border-indigo-100 rounded-[2.5rem] hover:border-indigo-400 hover:shadow-xl transition-all group">
                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-indigo-600 mb-5 shadow-sm group-hover:scale-110 transition-transform"><DownloadCloud size={32} /></div>
              ...
              <button onClick={() => setView("form")} className="flex flex-col items-center text-center p-10 bg-teal-50 border-2 border-teal-100 rounded-[2.5rem] hover:border-teal-400 hover:shadow-xl transition-all group">
                ...
                <p className="text-[10px] text-teal-600/60 font-bold mt-2 uppercase">Add custom rows</p>
              </button>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-2">
                    <button onClick={() => setRows([])} className="px-3 py-1.5 text-rose-500 text-[10px] font-black uppercase hover:bg-rose-50 rounded-lg transition-all flex items-center gap-1"><RotateCcw size={12} /> Reset</button>
                  ...
                  <div key={idx} className="p-4 bg-white border border-slate-200 rounded-[1.5rem] group hover:border-indigo-300 transition-all">
                    <div className="grid grid-cols-12 gap-3 items-center">
                      <div className="col-span-1">
                        <input type="number" value={row.order_index} onChange={(e) => handleChange(idx, "order_index", e.target.value)} className="w-full bg-slate-100 border-none rounded-lg py-2 text-center text-xs font-black text-slate-500 outline-none" />
                      </div>
                      <div className="col-span-6">
                        <input type="text" placeholder="Topic name..." value={row.topic} onChange={(e) => handleChange(idx, "topic", e.target.value)} className="w-full border-none focus:ring-0 px-1 py-1 text-sm font-bold text-slate-700 outline-none" />
                        <input type="text" placeholder="Description / Recipes..." value={row.description} onChange={(e) => handleChange(idx, "description", e.target.value)} className="w-full border-none focus:ring-0 px-1 text-[10px] font-medium text-slate-400 outline-none mt-1" />
                      </div>
                      <div className="col-span-4">
                        <select value={row.class_type} onChange={(e) => handleChange(idx, "class_type", e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-2 py-2 text-[11px] font-bold text-slate-600 outline-none">
                      ...
                      <div className="col-span-1 text-right">
                        <button onClick={() => setRows(rows.filter((_, i) => i !== idx))} className="p-2 text-slate-300 hover:text-rose-500 transition-colors"><Trash2 size={18} /></button>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {view === "form" && (
          <div className="p-6 bg-white border-t border-slate-100 flex justify-end gap-3 shrink-0">
            ...
            <button onClick={handleSubmit} disabled={isPending || rows.length === 0} className="px-10 py-3 bg-indigo-600 text-white text-xs font-black uppercase tracking-widest rounded-2xl hover:bg-indigo-700 disabled:opacity-50 transition-all flex items-center gap-2 shadow-xl shadow-indigo-100">
