...
export default function ClassSidebar({
  ...
  // Ensure viewTab falls back to 'daily' if it was set to the now-removed 'bazar' tab
      ...
      <div className="flex bg-white p-1 md:p-1.5 rounded-2xl shadow-sm border border-gray-100 mb-4 shrink-0">
        <button
          ...
          className={`flex-1 py-2 text-[10px] md:text-xs font-black uppercase tracking-widest rounded-xl transition-all ${
            currentTab === "daily" ? "bg-teal-500 text-white shadow-md" : "text-gray-500 hover:bg-gray-50"
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 text-[10px] md:text-xs font-black uppercase tracking-widest rounded-xl transition-all ${
            currentTab === "pending" ? "bg-amber-500 text-white shadow-md" : "text-gray-500 hover:bg-gray-50"
          }`}
        >
            ...
            {classesOnSelectedDate?.length > 0 ? (
              classesOnSelectedDate.map((cls) => (
                <div key={cls._id} className="group p-4 bg-white rounded-2xl border border-gray-100 shadow-sm relative hover:border-teal-100 transition-all flex flex-col">
                  ...
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end mt-auto pt-3 border-t border-gray-50 gap-3">
                    <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {/* REQUISITION / BAZAR BUTTON */}
                      <button 
                        onClick={() => onOpenRequisition(cls)} 
                        className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-2 bg-amber-50 text-amber-600 hover:bg-amber-100 rounded-lg transition-colors text-xs font-bold"
                      >
                        <ShoppingBag size={12} /> 
                      ...
                      <button 
                        ...
                        className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg transition-colors text-xs font-bold ${
                          cls.is_completed ? 'bg-gray-100 text-gray-600' : 'bg-teal-50 text-teal-600 hover:bg-teal-100'
                        }`}
                      >
                        <Users size={12} /> 
                        {cls.is_completed ? "View Roster" : "Attendance"}
                      </button>
                    </div>
                  </div>
                </div>
