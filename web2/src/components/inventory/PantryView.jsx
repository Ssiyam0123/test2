...
export default function PantryView({ branchId }) { 
  ...
  return (
    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl overflow-hidden animate-in fade-in flex flex-col h-full max-h-[600px]">
      <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center gap-2 text-slate-700 shrink-0">
      ...
      <div className="overflow-auto flex-1 custom-scrollbar">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-50">
            {inventory.length === 0 ? (
              <tr>
                <td colSpan="3" className="p-8 text-center text-slate-400 text-sm font-bold">No inventory items found.</td>
              </tr>
            ) : (
              inventory.map((item) => (
